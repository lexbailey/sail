...
(**
   In order to prefix all the global references, we "interecept" a SV pprint function [NameGen.to_string],
   and prepend when the thing to print is a global reference.
   To do so, we must have the list of globals somewhere, this is the purpose of below [list] variable.
   It contains the list of globals :
   - filled once at the "initialization point" when going through all global declarations
   - Used at each call of [NameGen.to_string] (the "interception point") to compare the variable to print against the globals.

   It is a mutable reference because it's the least impactful way (in terms of git rebasing)
   to bring it from the "initialization point" to the "interception point".
*)
let list : (Libsail__Ast.id * ctyp) list ref =
  ref []

(**
   Prepending should be done everywhere except in the sail_toplevel module (where globals are locally declared).
   To do so, we use below [active] switch and [toggle] function.
   When calling [pp_module], [toggle] will set [active] to [false] if and only if we're pprinting the [sail_toplevel] module.
*)
let active  : bool ref =
  ref true

let get_strlist () =
  List.map (fun (id, _) -> string_of_id id) !list
...
(**
   This is the function doing the actual prepending (and comparing against the global list),
   to be called at the "interception point".
*)
let prepend prefix_opt s =
  let prefix = match prefix_opt with Some s -> s ^ "." | None -> "" in
  if !active && (List.mem s (get_strlist ()))
  then prefix ^ s
  else s
...
(**
   This is the function used to diable prepending for the [sail_toplevel] module,
   to be called at the top of [pp_module]
*)
let toggle module_name =
  active := not @@ String.equal "sail_toplevel" module_name

(** Because of issue (4.) mentioned in the [Globals] doc above, we remove all output of the [sail_setup_let_] modules.

    This manual hack raises an exception in the List.map2 function which is called
    with a list of 1 returned value [my_global_1] and 0 output (the previously removed [my_global_1].

    This function intercepts and discards that exception since the behaviour is expected.
*)
let map2 f l1 l2 = match l1 with
...
module Gen = struct
  ...
  let list_with_dup global_prefix =
    if Option.is_none global_prefix then [] else (
      List.flatten @@ List.map (fun (id, ctyp) ->
          [ id, ctyp
          ; append_id id "_1", ctyp
          ]
        ) !list
    )

  let top_ids global_prefix =
    List.map (fun (id, _ctyp) -> name id) @@ list_with_dup global_prefix

  (** This function generates the list of declarations to add in [sail_toplevel] *)
  let top_defs global_prefix =
    List.map (fun (id, ctyp) -> SVD_var (name id, ctyp)) @@ list_with_dup global_prefix
  ...
  let always_comb_assignments global_prefix =
    if Option.is_none global_prefix then [] else List.map asgn !list

end (* module Gen *)

let remove_top_vars global_prefix in_module =
  Option.is_some global_prefix && Option.is_none in_module
